...
void createDungeon(std::shared_ptr<GameWorld> gameWorld, std::shared_ptr<Screen> screen, std::shared_ptr<Map> map, std::shared_ptr<GameObject> gameHandlerObject) {
	...
	if (gameHandlerObject != nullptr)
		...
		createMovingCharacter(gameWorld, glm::vec3(characterRoomCenter[0], characterRoom->room->getMinPoint()[1] + 2, characterRoomCenter[2]) + glm::vec3(0, 1, 0), map, characterRoom);
	// create goal
